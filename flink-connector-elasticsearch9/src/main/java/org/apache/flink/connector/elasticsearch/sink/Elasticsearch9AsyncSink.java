...
/**
 * Elasticsearch9AsyncSink Apache Flink's Async Sink that submits Operations into an Elasticsearch
 * cluster.
 *
 * @param <InputT> type of records that will be converted into {@link Operation} see {@link
 *     Elasticsearch9AsyncSinkBuilder} on how to construct valid instances
 */
public class Elasticsearch9AsyncSink<InputT> extends AsyncSinkBase<InputT, Operation> {
    ...
    public StatefulSinkWriter<InputT, BufferedRequestState<Operation>> createWriter(
            InitContext context) {
        return new Elasticsearch9AsyncWriter<>(
                getElementConverter(),
                context,
                getMaxBatchSize(),
    ...
    public SimpleVersionedSerializer<BufferedRequestState<Operation>> getWriterStateSerializer() {
        return new Elasticsearch9AsyncSinkSerializer();
    }
}
